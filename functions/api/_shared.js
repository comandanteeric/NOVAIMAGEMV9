...
function normalize(parsed={}){
  ...
  for(const k of [
    ...
    base[k]=Array.isArray(parsed[k]) ? parsed[k] : [];
  }

  base.priceConfig=
    parsed.priceConfig && typeof parsed.priceConfig==='object'
      ? parsed.priceConfig
      : null;
...
export async function writeDB(env,db){
  ...
  `)
  .bind(JSON.stringify(clean),clean.updatedAt)
  .run();
...
function secret(env){
  return env.NOVA_IMAGEM_SECRET ||
    'nova-imagem-erp-v8-change-this-secret';
}
...
export async function signToken(env,user){
  ...
  const sig=bytesToB64Url(await hmac(secret(env),body));
...
export async function verifyToken(env,request){
  try{
    ...
    const provided=b64UrlToBytes(sigText);
